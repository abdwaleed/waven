...
| --- | --- | --- | --- |
| `makeFilterLibrary` | `(nx, ny, n_orientations, n_sigmas, n_phases, nx * ny)` | `float16` | coupled size/frequency library for coarse RF |
| `makeFilterLibrary2` | `(NX, NY, n_orientations, n_sigmas, n_frequencies, n_phases, NX * NY)` | `float16` | independent-frequency fine library |
| `waveletDecomposition` | `(n_frames, coarse_nx, coarse_ny, n_orientations, n_sigmas)` | `float32` | one coarse phase at a time |
| `waveletDecompositionFull` | `(n_frames, NX, NY, n_orientations, n_sigmas, n_frequencies)` | `float32` | one full-model phase at a time |
