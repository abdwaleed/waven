...
`pipeline_config.json` is an optional, portable record of GUI inputs. The GUI can launch without it; missing keys use the normal GUI defaults and each action validates the fields it needs. I would say it is best to input the parameters and paths directly from the gui and then use `pipeline_config.json` as a reusable inputs cache to not rewrite everything upon launch. The safest workflow is:
...
JSON paths can use `{PROJECT_ROOT}` to avoid hardcoding absolute paths and favor paths relative to the root of this repo; Waven resolves it relative to the repository directory that contains `ui.py`. Additionally, should you populate a JSON manually, values may be stored as strings because they populate text fields. Waven parses numbers, lists, tuples, booleans, and paths when the relevant action runs. Finally, the JSON is structured as follows. Here is an example:
...
``` text
{
  ...
  "gabor_param": {
    ...
    "Phases": "[0, 90]",
    "Coarse Library Path": "{PROJECT_ROOT}/your_experiment/cache/gabor/coarse",
    "Fine Library Path": "{PROJECT_ROOT}/your_experiment/cache/gabor/full"
  },
  "common": {
...
| `gui` | object | Saved GUI choices: sampling, cache format, optional model flags, hardware controls, and export file types. |
| `gabor_param` | object | Orientation, scale, frequency, phase, and kernel-cache folder inputs. |
