...
In general, WavEn makes a cache for any intermediate output that is reusable. This saves processing time should you conduct the same analysis on different data. For example, a wavelet cache can be used on different raw data and stimulus videos. A cache is reused only
